...
- **UI 요구사항**:
  - 데이터가 로드되지 않은 초기 화면 중앙에 "여기로 CSV 또는 Excel 파일을 드래그 앤 드롭하세요" 라는 안내 문구와 아이콘(또는 점선 박스) 표시.
  - 파일 탐색기를 여는 '파일 선택' 버튼도 보조 수단으로 함께 제공.
- **기능 동작**:
  - 사용자가 지정된 영역에 파일을 드래그 앤 드롭하면, 파일 경로를 읽어 확장자 검사.
  - `.csv`, `.xlsx` 파일만 허용. 다른 포맷의 파일 드롭 시 오류 메시지(`QMessageBox` 등) 출력.
