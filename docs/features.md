...
- `gui/main_window.py`: 앱의 루트 윈도우. 설정 다이얼로그 진입점을 제공한다.
- `gui/settings_dialog.py`: API 키를 입력하고 저장하는 설정 다이얼로그.
- `config/config_manager.py`: `config.json` 파일에 설정을 읽고 쓰는 클래스.
...
- `gui/file_load_view.py`: 파일 선택 버튼과 안내 문구를 표시하는 초기 화면.
- `gui/main_window.py`: 파일 선택 이벤트를 받아 파싱 후 리스트 화면으로 전환하는 루트 윈도우.

## 5. 리뷰 데이터 조회 (카드 리스트 뷰)
...
- `gui/review_card_widget.py`: 리뷰 한 건을 카드 UI로 표현하는 위젯.
- `gui/review_list_view.py`: 카드 위젯 목록을 세로로 스크롤할 수 있게 배치하는 뷰.
...
- `ai/client.py`: AI 답글 생성 클라이언트의 추상 인터페이스 및 예외 정의.
- `ai/openai_client.py`: LangChain을 사용하여 OpenAI API를 호출하는 클라이언트 구현체.
- `ai/worker.py`: AI 답글 생성을 백그라운드 스레드에서 비동기로 처리하는 워커.
- `gui/review_card_widget.py`: 답글 생성 버튼, 로딩 상태, 결과 출력, 오류 표시.
- `gui/review_list_view.py`: 각 카드에 AI 클라이언트를 주입하여 목록을 구성.
...
- `gui/review_card_widget.py`: 클립보드 복사 버튼과 복사 완료 피드백을 처리 기능을 포함.
